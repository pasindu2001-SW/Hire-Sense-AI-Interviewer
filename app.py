...
def display_final_results():
    """Display final interview results with hire/no hire decision"""
    if (
    ...
    # Determine hire/no hire decision based on score
    hire_threshold = 6.0  # You can adjust this threshold as needed
    is_hired = final_score >= hire_threshold
    
    # Display results with hire decision
    st.subheader("🎉 Interview Results")
    st.markdown(f"**Candidate:** {st.session_state['name']}")
    st.markdown(f"**Overall Score:** {final_score:.2f}/10")
    
    # Display hire decision with appropriate styling
    if is_hired:
        st.success("✅ **Decision: HIRE**")
        st.markdown("🎊 **Congratulations!** You've performed well in the interview.")
    else:
        st.error("❌ **Decision: NO HIRE**")
        st.markdown("📈 **Keep practicing!** Review the feedback below to improve your interview skills.")
    
    # Show performance level
    if final_score >= 8.5:
        performance_level = "Excellent"
        st.markdown("🌟 **Performance Level:** Outstanding candidate")
    elif final_score >= 7.0:
        performance_level = "Very Good"
        st.markdown("⭐ **Performance Level:** Strong candidate")
    elif final_score >= 6.0:
        performance_level = "Good"
        st.markdown("✨ **Performance Level:** Acceptable candidate")
    elif final_score >= 4.0:
        performance_level = "Fair"
        st.markdown("⚡ **Performance Level:** Needs improvement")
    else:
        performance_level = "Poor"
        st.markdown("📚 **Performance Level:** Significant improvement needed")

    # Show detailed summary
    st.subheader("Detailed Interview Summary")
    for i, conv in enumerate(st.session_state["conversations"], 1):
        # Color code based on individual question scores
        ...
        with st.expander(f"{score_color} Question {i} (Score: {conv['Evaluation']}/10)"):
